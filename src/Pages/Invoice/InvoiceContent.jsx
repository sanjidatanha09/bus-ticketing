...
const InvoiceContent = () => {
  ...
  const { saveBookingData, searchTicket, unBookingInfo } =
    useContext(BusProvider);
  ...
  const fetchData = async () => {
    setLoading(true);
    const pnrInfo = {
      pnr: saveBookingData?.invoiceData?.invoice || searchTicket.pnr,
      contact_number:
        saveBookingData?.invoiceData?.single_ticketing_data.contact_number ||
        searchTicket.contact_number,
    };
    try {
  ...
  const fetchUnData = async () => {
    setLoading(true);
    const pnrInfo = {
      pnr: unBookingInfo.pnr_no,
      contact_number: unBookingInfo.contact_number,
    };
    try {
      const res = await axiosPublic.post(
  ...
  return (
    <div className="max-w-7xl mx-auto border bg-primary4 flex relative">
          ...
          {invoiceInfo?.single_ticketing_data?.trip_data?.is_unlimited_trip !==
            ...
            <ul className="text-left  w-full">
              <li className="text-lg font-semibold flex justify-between">
                ...
                <span className="w-7/12">{invoiceInfo?.single_ticketing_data?.contact_name}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">Phone</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">{invoiceInfo?.single_ticketing_data?.contact_number}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">Bus Name</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">{invoiceInfo?.trip_data?.bus_data?.bus_name}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">Seats</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">{invoiceInfo?.ticketing_data?.map((item) => item.seat_no)}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
              ...
              <li className="text-lg font-semibold flex justify-between">
                ...
                <span className="w-1/12">:</span>
                <span className="w-7/12">{invoiceInfo?.ticketing_data?.map((item) => item.seat_price)}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
            ...
            <ul className="text-left w-full">
              ...
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">Seat Fare</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">{invoiceInfo?.trip_data.seat_price}/-</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">Total Fare</span>
                <span className="w-1/12">:</span>
              ...
              <li className="text-lg font-semibold flex justify-between">
                ...
                <span className="w-1/12">:</span>
                <span className="w-7/12"> {unBookingInfo?.center_name}-</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">PNR</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">  {invoiceInfo?.single_ticketing_data.pnr_no}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">From</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">  {invoiceInfo?.get_boarding_point_data?.map(
                  (item) => item.point_name
                )}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">To</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">   {invoiceInfo?.get_dropping_point_data?.map(
                  (item) => item.point_name
                )}</span>
              </li>
              <li className="text-lg font-semibold flex justify-between">
                <span className="w-4/12">Issued On</span>
                <span className="w-1/12">:</span>
                <span className="w-7/12">{invoiceInfo?.single_ticketing_data.created_at}</span>
              </li>
            </ul>
          )}
        </div>
      ...
      <section className="w-2/3 bg-primary2 pr-3 relative">
              ...
              {invoiceInfo?.single_ticketing_data?.trip_data
                ...
                <ul className="text-left space-y-1 w-full ">
                  <li className="text-lg font-semibold flex justify-between">
                    ...
                    <span className="w-1/12">:</span>
                    <span className="w-6/12"> {invoiceInfo?.single_ticketing_data?.contact_name}</span>
                  </li>
                  <li className="text-lg font-semibold flex justify-between">
                    <span className="w-5/12">Coach</span>
                    <span className="w-1/12">:</span>
                    <span className="w-6/12"> {invoiceInfo?.trip_data.bus_data?.bus_name}</span>
                  </li>
                  <li className="text-lg font-semibold flex justify-between">
                    <span className="w-5/12">Departure Time</span>
                    <span className="w-1/12">:</span>
                    <span className="w-6/12">  {invoiceInfo?.trip_data?.deperture_time}</span>
                  </li>
                  <li className="text-lg font-semibold flex justify-between">
                    <span className="w-5/12">Return Time</span>
                    <span className="w-1/12">:</span>
                    <span className="w-6/12">  {invoiceInfo?.trip_data?.return_time}</span>
                  </li>

                  <li className="text-lg font-semibold flex justify-between">
                    <span className="w-5/12">Seat Fare</span>
                    <span className="w-1/12">:</span>
                    <span className="w-6/12">  {invoiceInfo?.ticketing_data?.map(
                  ...
                  <li className="text-lg font-semibold flex justify-between">
                    ...
                    <span className="w-1/12">:</span>
                      <span className="w-6/12"> {invoiceInfo?.ticketing_data?.map((item) => item.seat_no)}</span>
                  </li>
                
                  <li className="text-lg font-semibold flex justify-between">
                    <span className="w-5/12">Boarding</span>
                    <span className="w-1/12">:</span>
                      <span className="w-6/12">  {invoiceInfo?.get_boarding_point_data?.map(
                        (item) => item.point_name
                      )} </span>
                  </li>
                  <li className="text-lg font-semibold flex justify-between">
                    ...
                    <span className="w-1/12">:</span>
                      <span className="w-6/12"> {invoiceInfo?.single_ticketing_data?.created_at}</span>
                  </li>
                </ul>
              ) : (
              ...
              {invoiceInfo?.single_ticketing_data?.trip_data
                ...
                <ul className="text-left w-full space-y-1">
                    ...
                    <li className="text-lg font-semibold flex justify-between">
                      ...
                      <span className="w-1/12">:</span>
                      <span className="w-7/12">  {invoiceInfo?.trip_data?.deperture_date}</span>
                    </li>
                    <li className="text-lg font-semibold flex justify-between">
                    ...
                    <li className="text-lg font-semibold flex justify-between">
                     ...
                     <li className="text-lg font-semibold flex justify-between">
                      ...
                      <span className="w-1/12">:</span>
                      <span className="w-7/12">  {invoiceInfo?.single_ticketing_data?.ticketing_by
                        ? invoiceInfo?.single_ticketing_data?.ticketing_by
                        : "Online"}</span>
                    </li>
                    <li className="text-lg font-semibold flex justify-between">
                ...
                <ul className="text-left w-full space-y-1">
                    ...
                    <li className="text-lg font-semibold flex justify-between">
                      ...
                      <span className="w-1/12">:</span>
                      <span className="w-7/12">{invoiceInfo?.single_ticketing_data.trip_data.exam_date}</span>
                    </li>
                    <li className="text-lg font-semibold flex justify-between">
                      <span className="w-5/12">Journey Date</span>
                      <span className="w-1/12">:</span>
                    ...
                    <li className="text-lg font-semibold flex justify-between">
                      ...
                      <span className="w-1/12">:</span>
                      <span className="w-7/12">  {invoiceInfo?.single_ticketing_data.trip_data.deperture_time}</span>
                    </li>
                    <li className="text-lg font-semibold flex justify-between">
                      <span className="w-5/12">Return Date</span>
                      <span className="w-1/12">:</span>
                      <span className="w-7/12">  {invoiceInfo?.single_ticketing_data.trip_data.return_date}</span>
                    </li>
                    <li className="text-lg font-semibold flex justify-between">
                      <span className="w-5/12">Return Time</span>
                      <span className="w-1/12">:</span>
                      <span className="w-7/12">  {invoiceInfo?.single_ticketing_data.trip_data.return_time}</span>
                    </li>
                    <li className="text-lg font-semibold flex justify-between">
        ...
        <p className="text-center">Printed By: <span>{invoiceInfo?.single_ticketing_data?.ticketing_by
          ? invoiceInfo?.single_ticketing_data?.ticketing_by
          : "Online"}</span></p>
